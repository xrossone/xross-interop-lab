...
- **Source IDs**：R01–R19（intake 与 dossier）、xross-dev@099b6c72（映射）、
  localsend-rs@4f46cc1（用户副本）；全部与 `references/sources.lock.json` 一致（测试强制）。
...
| localsend-rs LICENSE 文件 | 用户自有仓库缺 LICENSE 文件（Cargo.toml 声明 MIT） | 用户补文件 | → approved |
...
- **需 contract owner（用户）裁决的唯一问题**：**是否批准 T05 的 provider-adoption 放行**
  （当前全部 `production_approved=false`）。其中 localsend reuse 只差 LICENSE 文件补齐；
  shairplay-rust worker 需要 LGPL 链接形态的最终确认（动态 vs 独立进程参数化）。

## 下一阶段建议（T11–T13 + 首批 provider 接入）

1. **T13 worker supervisor + 真实隔离 probe**：为 AirPlay worker（LGPL 单元）铺路；
